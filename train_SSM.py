...
from model_SMM import Encoder, StateSpaceModel, ObservationModel, RewardModel
from utils import ReplayBuffer, preprocess_obs
from wrappers import GymWrapper, RepeatAction


def save_checkpoint(encoder, ssm, obs_model, reward_model, log_dir, episode):
    ...
    torch.save(ssm.state_dict(), os.path.join(checkpoint_dir, 'ssm.pth'))
...
def main():
    ...
    ssm = StateSpaceModel(args.state_dim,
                          env.action_space.shape[0],
                          embed_size=args.embed_size).to(device)
    obs_model = ObservationModel(args.state_dim).to(device)
    reward_model = RewardModel(args.state_dim).to(device)
    ...
    for episode in range(args.seed_episodes, args.all_episodes):
        # collect experiences
        start = time.time()
        # Note: CEMAgent expects RSSM interface, so we use random actions for SSM training
        # In practice, you might want to create an SSM-specific agent
        from agent import CEMAgent
        try:
            cem_agent = CEMAgent(encoder, ssm, reward_model,
                                 args.horizon, args.N_iterations,
                                 args.N_candidates, args.N_top_candidates)
        except:
            # Fallback to random actions if agent doesn't work with SSM
            cem_agent = None
        ...
        for update_step in range(args.collect_interval):
            ...
            # prepare Tensor to maintain states sequence (no RNN hidden states for SSM)
            states = torch.zeros(
                args.chunk_length, args.batch_size, args.state_dim, device=device)

            # initialize state with 0 vector
            state = torch.zeros(args.batch_size, args.state_dim, device=device)

            # compute state sequences and kl loss
            ...
            # Equation 3: Accumulates the sum over time steps
            kl_loss = 0
            for l in range(args.chunk_length-1):
                next_state_prior, next_state_posterior = \
                    ssm(state, actions[l], embedded_observations[l+1])
            ...
            # SSM uses only state (no RNN hidden state)
            flatten_states = states.view(-1, args.state_dim)
            recon_observations = obs_model(flatten_states).view(
                args.chunk_length, args.batch_size, 3, 64, 64)
            predicted_rewards = reward_model(flatten_states).view(
        ...
        if (episode + 1) % args.test_interval == 0:
            ...
            if cem_agent is not None:
                try:
                    test_agent = CEMAgent(encoder, ssm, reward_model,
                                         args.horizon, args.N_iterations,
                                         args.N_candidates, args.N_top_candidates)
                    obs = env.reset()
                    done = False
        ...
        if (episode + 1) % args.checkpoint_interval == 0:
            save_checkpoint(encoder, ssm, obs_model, reward_model, log_dir, episode)

    # save learned model parameters (final checkpoint)
    save_checkpoint(encoder, ssm, obs_model, reward_model, log_dir, args.all_episodes - 1)
    writer.close()
